...
getFollows(id, page){
  this._followService.getFollower(this.token, id, page).subscribe(
    response =>{
    if(!response.follows){
      this.status = 'error'
    }else{
    ...
    //this.following = response. users_follow_me;

    console.log('a mostar '  +  this.following);
      
  if(page > this.pages){
    this._router.navigate(['/seguidores',id, 1]);
    }
      }
    },
    error =>{
